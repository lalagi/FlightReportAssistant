...
The application stores all processed data in an SQLite database within a single table named `flight_reports`. This table holds both the original event data and the AI-generated enrichments.

| Column                      | Type       | Description                                                                    |
| :-------------------------- | :--------- | :----------------------------------------------------------------------------- |
| `id`                        | TEXT       | Primary key, a unique identifier (UUID) for each record.                       |
| `timestamp`                 | TEXT       | The event's timestamp from the source file.                                    |
| `source`                    | TEXT       | The name of the original file where the data originated (e.g., `event_ops.json`). |
| `raw_event_text`            | TEXT       | The original, unprocessed event description.                                   |
| `summary`                   | TEXT       | The one-sentence summary generated by the AI.                                  |
| `category`                  | TEXT       | The category determined by the AI (e.g., "Avionics", "Mechanical").            |
| `severity`                  | TEXT       | The severity level determined by the AI (e.g., "low", "high").                 |
| `recommendation`            | TEXT       | The action or recommendation suggested by the AI.                              |
| `model_meta`                | TEXT       | A JSON string containing metadata about the AI model used for processing.      |
| `UNIQUE(timestamp, raw_event_text)` | CONSTRAINT | A constraint to prevent the insertion of duplicate records.                    |
