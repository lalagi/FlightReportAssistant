...
class HuggingFaceAIService(AIService):
    ...
    def _generate_recommendation(
        self, raw_event_text: str, category: str, severity: str
    ) -> str:
...
class MockAIService(AIService):
    ...
    def process_text(self, raw_event_text: str) -> Dict[str, Any]:
        ...
        if any(keyword in text_lower for keyword in ["bird strike", "damage", "fire"]):
            ...
            for keyword in ["engine", "hydraulic", "apu", "pressure", "vibration"]
        ):
            category = "Mechanical"
            severity = "high"
            ...
            keyword in text_lower
            for keyword in ["landing gear", "tire", "brakes", "slats"]
        ):
            category = "Flight Controls"
            severity = "high"
            recommendation = (
                "Inspect relevant flight control systems before next flight."
            )
        elif any(
            keyword in text_lower
            for keyword in [
            ...
            keyword in text_lower
            for keyword in ["weather", "turbulence", "wind shear", "gusts", "storm"]
        ):
            category = "Weather"
            severity = "medium"
            ...
            severity = "low"
            recommendation = (
                "Add to next training session for crew resource management."
            )
