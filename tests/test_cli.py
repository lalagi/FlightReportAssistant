...
def test_init_command(runner, mocker):
    ...
    mock_db = mocker.patch('assistant.database_handler.get_database_handler').return_value
...
def test_list_command(runner, mocker):
    ...
    mock_db.list_reports_by_severity.return_value = [
        {"id": "1", "timestamp": "T1", "category": "C1", "summary": "S1", "recommendation": "R1"}
    ]
...
def test_show_command(runner, mocker):
    ...
    mock_db = mocker.patch('assistant.database_handler.get_database_handler').return_value
    mock_db.get_report_by_id.return_value = {"id": "xyz", "summary": "Detailed summary here"}
